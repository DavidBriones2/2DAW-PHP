...
class AccesoDatos {
   ...
   // Constructor privado  Patron singleton
    ...
    public function getProducto (String $produ) {
        $product = false;
        ...
        $stmt_producto->bind_param("s",$produ);
    ...
    public function addProducto($product):bool{
        ...
        $stmt_creaproduct  = $this->dbh->prepare("insert into PRODUCTOS (DESCRIPCION, PRODUCTO_NO, PRECIO_ACTUAL, STOCK_DISPONIBLE) Values(?,?,?,?)");
        if ( $stmt_creauser == false) die ($this->dbh->error);

        $stmt_creaproduct->bind_param("ssss",$product->DESCRIPCION, $product->PRODUCTO_NO, $product->PRECIO_ACTUAL, $product->STOCK_DISPONIBLE);
    ...
    //DELETE
    public function borrarProducto(String $product):bool {
        $stmt_boruser   = $this->dbh->prepare("delete from PRODUCTOS where PRODUCTO_NO=?");
        if ( $stmt_boruser == false) die ($this->dbh->error);
       
        $stmt_borproduct->bind_param("s", $product);
